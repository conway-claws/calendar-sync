#!/usr/bin/env python3
"""Build docs/claws.ics (and one sub-feed per label) from the three sources.
...
Usage:
  ...
  python3 bin/build.py --offline DIR  # read ical.ics/feed.json/doc.txt from DIR
...
import ical
import sources
from config import CALNAME, LABELS, REMOVAL_CAP, UID_DOMAIN, WINDOW_DAYS

DOCS = Path(__file__).resolve().parent.parent / "docs"
PRIORITY = {"ical": 3, "doc": 2, "feed": 1, "rss": 0}
...
def gather(offline_dir, today):
    ...
    if offline_dir:
        d = Path(offline_dir)
        ical_text = (d / "ical.ics").read_text() if (d / "ical.ics").exists() else None
        feed_posts = None
        ...
        ical_text = sources.fetch_ical()
        feed_posts = sources.fetch_feed_posts()
        rss_posts = sources.fetch_rss_posts()
        doc_text = sources.fetch_doc()

    if ical_text is None:
        per_source["ical"] = None
        notes.append("ical: fetch FAILED, carrying previous events forward")
    else:
        evs = [e for e in ical.parse_ics(ical_text) if in_window(e, today)]
        for e in evs:
            e["source"] = "ical"
        per_source["ical"] = evs
